...
class FakeGameState {
    ...
    isExploration() { return this.#state === "EXPLORATION"; }
    isHaunt() { return this.#state === "HAUNT"; }
...
class FakeGraphMap {
    ...
    hasRoom(roomId) { return this.#rooms.has(roomId); }
...
export class RegressionContextFactory {

    static createContext(overrides = {}) {
        const players = overrides.players ?? new FakePlayerManager();
        const gameState = overrides.gameState ?? new FakeGameState();
        const graphMap = overrides.graphMap ?? new FakeGraphMap();
        const cardManager = overrides.cardManager ?? new FakeCardManager();

        return new ScenarioContext({ players, gameState, graphMap, cardManager });
    }

    static createRouter() {
        return new InformationRouter();
    }
}